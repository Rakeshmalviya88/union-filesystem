...
#include <fuse.h>
#else
#error "FUSE headers not found. Install libfuse3-dev (or libfuse-dev)."
#endif
#else
#include <fuse.h>
#endif
 ...
 */
static int ensure_parent_dirs(const char *path) {             // path is the file name we want to create/modify, we need to ensure its parent directories exist in upper layer
    struct mini_unionfs_state *state = UNIONFS_DATA;
    char *path_copy = strdup(path);
    char *dir = dirname(path_copy);
...
static int unionfs_open(const char *path, struct fuse_file_info *fi) {
    ...
    /* If writing to a file in lower layer, perform CoW , If the file is in the read-only lower layer AND you're opening it for writing → copy it to upper first, then point resolved_path at the upper copy.
Read-only opens skip this entirely.*/
    ...
    int fd = open(resolved_path, fi->flags);    // fi->flags contains the flags passed to open() (like O_RDONLY, O_WRONLY, etc), we use it to check if we can open the file with the requested permissions
    if (fd < 0) {
        return -errno;
    }
    close(fd);
...
static int unionfs_read(const char *path, char *buf, size_t size, off_t offset,
                        struct fuse_file_info *fi) {
                            // path is the file name we want to read, (ex: path = file1.txt)
    (void) fi;
    char resolved_path[PATH_MAX];
    path_location_t loc;
    int fd;
    ...
    loc = resolve_path(path, resolved_path, sizeof(resolved_path));   // Asks: where does this file live? Upper, lower, whiteout, or nowhere?
 
    if (loc == PATH_WHITEOUT || loc == PATH_NONE) {
        return -ENOENT;         // enoent means "No such file or directory"
    }
    
    fd = open(resolved_path, O_RDONLY);         // O_RDONLY means open for read only
    if (fd < 0) {
        return -errno;
    }
    
    res = pread(fd, buf, size, offset);      // res means how many bytes were actually read, or -1 if there was an error
    if (res < 0) {                 // pread means "read from a file descriptor at a given offset", it's like read() but you can specify where to read from without changing the file pointer
        res = -errno;
    }
...
static int unionfs_write(const char *path, const char *buf, size_t size,
    ...
    get_upper_path(path, upper_path, sizeof(upper_path));    // Get the path to the file in the upper layer (after CoW, it should be there)
    
    fd = open(upper_path, O_WRONLY);     // O_WRONLY means open for write only
    if (fd < 0) {
        return -errno;
    }
    
    res = pwrite(fd, buf, size, offset);    // pwrite means "write to a file descriptor at a given offset", it's like write() but you can specify where to write without changing the file pointer
    if (res < 0) {
        res = -errno;
    }
...
static int unionfs_unlink(const char *path) {
    char resolved_path[PATH_MAX];   // resolved_path is the actual path to the file we want to delete, 
    char upper_path[PATH_MAX];    // path to the file in the upper layer
    char whiteout_path[PATH_MAX];
    path_location_t loc;
    int res;    // res is used to store the result of system calls, if it's negative, it means there was an error and the value is -errno
    ...
    get_upper_path(path, upper_path, sizeof(upper_path));   // Get the path to the file in the upper layer (if it exists)
...
static int unionfs_mkdir(const char *path, mode_t mode) {
    ...
    /* Ensure parent directories exist */
    res = ensure_parent_dirs(path);   // Ensure that the parent directories of the directory we want to create exist in the upper layer, if not, create them. This is necessary because we are creating a directory in the upper layer, and it needs to have its parent directories in place.
    if (res != 0) {
        return res;
    }
...
static int remove_whiteouts_in_dir(const char *upper_dir_path) {
    ...
    while ((de = readdir(dp)) != NULL) {    // Iterate through all entries in the directory
        if (strcmp(de->d_name, ".") == 0 || strcmp(de->d_name, "..") == 0) {
            continue;
        }
...
static int unionfs_chmod(const char *path, mode_t mode,
    ...
    res = chmod(upper_path, mode);   // chmod means "change the permissions of a file", it takes the path to the file and the new mode (permissions) we want to set
    if (res != 0) {
        return -errno;
    }
